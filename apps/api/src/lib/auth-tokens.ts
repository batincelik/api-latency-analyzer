...
function accessSecret(env: AppEnv): Uint8Array {
  return encoder.encode(env.JWT_ACCESS_SECRET);
}

function refreshSecret(env: AppEnv): Uint8Array {
...
export async function signRefreshJwt(env: AppEnv, sessionId: string): Promise<string> {
...
export async function verifyRefreshJwt(env: AppEnv, token: string): Promise<{ sessionId: string }> {
  const { payload } = await jwtVerify(token, refreshSecret(env));
  const sessionId = payload.sub;
  if (typeof sessionId !== 'string') throw new Error('Invalid refresh token');
  return { sessionId };
}
