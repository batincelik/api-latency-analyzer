...
function windowBounds(
  ...
  if (window === 'custom') {
    if (!from || !to) throw AppError.validation({ window: 'custom requires from and to' });
    return { start: from, end: to };
  }
  const start = new Date(end);
  if (window === '1h') start.setHours(start.getHours() - 1);
  else if (window === '24h') start.setHours(start.getHours() - 24);
...
async function computeIncidentStreak(
  endpointId: string,
  start: Date,
  end: Date,
): Promise<{ openFailures: number; longestFailureStreak: number }> {
