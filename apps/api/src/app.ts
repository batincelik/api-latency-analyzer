import { loadEnv, type AppEnv } from '@ala/config';
...
export async function buildApp(options: { env?: AppEnv; redis: Redis }) {
...
function parseBodyLimitBytes(limit: string): number {
  const m = /^(\d+)(kb|mb)?$/i.exec(limit.trim());
  if (!m) return 256 * 1024;
  const n = Number(m[1]);
