import { AlertRuleType, AuditAction, HttpMethod, prisma } from '@ala/db';
import { AppError } from '../lib/app-error.js';
import { writeAudit } from './audit.service.js';
function mapMethod(m) {
    return HttpMethod[m];
}
export async function createEndpoint(userId, body, meta) {
...
export async function listEndpoints(userId, query) {
    const where = {
        userId,
...
export async function updateEndpoint(userId, id, body, meta) {
    ...
    if (body.method !== undefined)
        data.method = mapMethod(body.method);
    ...
    if (body.expectedStatusCodes !== undefined)
        data.expectedStatusCodes = body.expectedStatusCodes;
...
export async function softDeleteEndpoint(userId, id, meta) {
    const existing = await prisma.endpoint.findFirst({ where: { id, userId } });
    if (!existing)
        throw AppError.notFound('Endpoint');
