import { CheckErrorClass, prisma } from '@ala/db';
...
export function startMonitorWorker(env, connection) {
    const worker = new Worker(CHECK_QUEUE, async (job) => {
...
async function runCheckWithRetries(env, ep, correlationId, jobId) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        const retryable = single.errorClass === CheckErrorClass.NETWORK ||
        ...
        log.warn({ endpointId: ep.id, correlationId, jobId, attempt, backoff }, 'check_retry_backoff');
        await delay(backoff);
    }
    return last ?? {
        success: false,
...
async function runSingleCheck(ep) {
...
async function evaluateAndFireAlerts(ep, latestSuccess, latestLatency, correlationId) {
...
async function resolveOpenAlert(alertRuleId, dedupeKey) {
    const open = await prisma.alertEvent.findFirst({
