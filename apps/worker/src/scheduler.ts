...
import { CHECK_QUEUE, SCHEDULER_QUEUE, type CheckJobData } from './queues.js';

const log = createLogger({ name: 'worker-scheduler' });

export async function startScheduler(env: AppEnv, connection: Redis): Promise<() => Promise<void>> {
  const checkQueue = new Queue<CheckJobData>(CHECK_QUEUE, { connection });
    ...
    async () => {
      ...
      for (const ep of due) {
        ...
        await checkQueue.add(
          'run-check',
          { endpointId: ep.id, correlationId },
          {
